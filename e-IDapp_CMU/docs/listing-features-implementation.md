...
### New Storage Keys:
```typescript
CONNECTIONS_DATA: 'e-id_connections_data',
CREDENTIALS_DATA: 'e-id_credentials_data',
```
...
await dispatch(initializeAgent({
  label: storedUserData.name || userData.name || 'e-ID Holder',
  pin: pin,
  endpoints: []
})).unwrap();
...
### Storage Keys:
- New keys: `e-id_connections_data`, `e-id_credentials_data`
