# e-ID Deep Linking (Android)

The e-ID app supports deep linking via the `e-id://` scheme on Android. This allows users to open invitations (connections, credentials, and proofs) directly from web links, emails, SMS, or other apps.

## URL Scheme Specification

The primary scheme is `e-id://`.

### Supported URL Patterns

| Type                   | URL Pattern                                    | Purpose                                                     |
| :--------------------- | :--------------------------------------------- | :---------------------------------------------------------- |
| **Generic Invitation** | `e-id://invite?oob=<base64_invitation>`      | Automatically detects the invitation type from the payload. |
| **Connection Request** | `e-id://invite?oob=<base64>&type=connection` | Explicitly triggers the connection request modal.           |
| **Credential Offer**   | `e-id://invite?oob=<base64>&type=offer`      | Explicitly triggers the credential offer modal.             |
| **Proof Request**      | `e-id://invite?oob=<base64>&type=proof`      | Explicitly triggers the proof request modal.                |
| **ZKP Proof Request**  | `e-id://invite?oob=<base64>&type=zkp-proof`  | Explicitly triggers the ZKP proof request flow.             |

## How it Works

1.  **Android Intent Filter**: `MainActivity` is registered to handle `e-id://` links in `AndroidManifest.xml`.
...
```bash
adb shell am start -W -a android.intent.action.VIEW -d "e-id://invite?oob=eyAiQGlkIjogIjEyMyIsICJAdHlwZSI6ICJodHRwczovL2RpZGNvbW0ub3JnL291dC1vZi1iYW5kLzEuMC9pbnZpdGF0aW9uIiwgImxhYmVsIjogIlRlc3QgT3JnYW5pemF0aW9uIiB9"
```
...
```bash
adb shell am start -W -a android.intent.action.VIEW -d "e-id://invite?oob=YOUR_BASE64_HERE&type=offer"
```
...
2. Type `e-id://invite?oob=...` into the address bar and press Enter.
3. Chrome should prompt to "Open with e-ID".
...
Most chat apps (WhatsApp, Slack, etc.) do not automatically make `e-id://` links clickable. To handle this, use a **Web Redirector**:
...
2.  **Add a redirect script** to that page:
    ...
    window.onload = function() {
      ...
      if (oob) {
        window.location.href = "e-id://invite?oob=" + oob;
      }
    };
    ```
...
*   **App not opening**: Ensure the `e-id` scheme is correctly registered in `AndroidManifest.xml`.
