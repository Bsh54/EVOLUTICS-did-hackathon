...
import { extractLabelParam, extractTypeParam, resolveShortUrl } from '../utils/invitationDecoder';
...
export default class CredoAgentService {
  ...
  async acceptInvitation(invitationUrl: string): Promise<ConnectionType> {
    if (!this.isInitialized || !this.agent) throw new Error('Agent not initialized');
    try {
      // Resolve short URL to full invitation URL before passing to Credo
      console.log('Original invitation URL:', invitationUrl);
      const resolvedUrl = await resolveShortUrl(invitationUrl);
      console.log('Resolved invitation URL:', resolvedUrl);

      // Extract URL parameters (type and label) from the resolved URL
      const urlLabel = extractLabelParam(resolvedUrl);
      const urlType = extractTypeParam(resolvedUrl);
