      ...
      if (codes?.length) {
        ...
        if (value) {
          ...
          try {
            ...
            // Use the resolved full URL (with ?oob=) instead of the raw short URL
            // This is critical: the raw QR value is a short URL without ?oob= param
            // which Credo SDK cannot parse. details.fullUrl is already resolved.
            if (details.fullUrl && details.fullUrl !== value) {
              console.log('📌 [PROCESS] Using resolved full URL instead of raw short URL');
              setScannedUrl(details.fullUrl);
            }
