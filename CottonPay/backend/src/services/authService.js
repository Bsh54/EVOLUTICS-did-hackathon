...
class AuthService {
  ...
  buildAuthorizationUrl({ codeChallenge, state, nonce }) {
    // Demander explicitement les claims essentiels via le paramètre OIDC claims
  ...
  async exchangeCodeForTokens(code, codeVerifier) {
    try {
      ...
      console.error('❌ Token exchange error:', { status, details });
      // Remonter un message plus explicite selon le type d'erreur
      if (error.response?.data?.error === 'invalid_client') {
        throw new Error('Échec de l\'échange du code : client OIDC invalide ou clés désynchronisées');
      }
      throw new Error(`Échec de l'échange du code contre des jetons (HTTP ${status || 'timeout'})`);
    }
  }
