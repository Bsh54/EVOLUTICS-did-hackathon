...
export class CredoAgentService {
  ...
  async initializeAgent(input: {
    ...
    // DID Registration (before caching the agent, so failures allow retry)
    ...
    try {
      ...
      console.warn('⚠️ Mediator connection failed (non-fatal):', (mediatorErr as any)?.message);
    }

    // Register events (single call)
    this.eventsService.registerEventHandlers(agent);

    // Cache agent ONLY after successful initialization + DID registration
    this.agent = agent;
